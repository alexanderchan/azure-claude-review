...
program
  ...
  .option('-p, --prompt-file <file>', 'Custom prompt file', path.join(__dirname, '..', '..', 'prompts', 'pr-review-prompt.md'))
  ...
  .option('--azure-pr <id>', 'Azure DevOps PR ID (will auto-detect if not provided)')
  .option('--use-env-vars', 'Use environment variables instead of Azure CLI auto-detection')
  .option('--use-existing-review', 'Use existing claude-review.md file instead of running Claude again')
  .option('--append', 'Append to existing Claude review comment instead of replacing it')
  .option('--new-comment', 'Always create a new comment instead of updating existing one')
  .parse();
...
async function main() {
  try {
    ...
    try {
      execSync(`git rev-parse --verify ${options.compareBranch}`, { stdio: 'ignore' });
    } catch (error) {
      console.error(chalk.red(`❌ Branch '${options.compareBranch}' not found`));
      process.exit(1);
    }
    ...
    if (!options.noPost) {
      ...
      if (azureConfig) {
        console.log(chalk.gray(`Found PR: ${azureConfig.org}/${azureConfig.project} - PR #${azureConfig.prId}`));
        ...
        console.log(chalk.yellow('⚠️  No Azure DevOps PR detected. Use --azure-pr <id> or set AZURE_DEVOPS_PR_ID'));
      }
    }
...
function processClaudeOutput(reviewFile) {
  if (!fs.existsSync(reviewFile)) {
    throw new Error('Claude did not create the review file. Make sure Claude has write permissions.');
  }
...
async function getAzureDevOpsConfig() {
  ...
  try {
    ...
    console.log(chalk.gray('Azure CLI detection failed, trying environment variables...'));
  }
...
function parseAzurePr(pr) {
  try {
    ...
    if (!urlMatch) {
      console.log(chalk.yellow('⚠️  Could not extract organization from PR URL'));
      return null;
    }
    ...
    if (!token) {
      console.log(chalk.yellow('⚠️  AZURE_DEVOPS_TOKEN environment variable is required'));
      return null;
    }
...
async function findExistingClaudeComment(config) {
  ...
  try {
    ...
    console.log(chalk.gray(`Could not search for existing comments: ${error.message}`));
    return null;
  }
}
...
async function postToAzureDevOps(reviewContent, config) {
  ...
  try {
    ...
    if (response.statusCode >= 200 && response.statusCode < 300) {
      ...
      console.error(chalk.red(`❌ Failed to post review: ${response.statusCode} ${response.statusMessage}`));
      console.error('Response body:', response.body);
    }
  } catch (error) {
    console.error(chalk.red(`❌ Error posting to Azure DevOps: ${error.message}`));
  }
}
